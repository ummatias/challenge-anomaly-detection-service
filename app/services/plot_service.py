...
class PlotService:
    ...
    def plot(self, series_id: str, version: str | None = None) -> str:
        ...
        fig = go.Figure()
        fig.add_scatter(
            x=[t for t, _ in normal], y=[v for _, v in normal],
            mode="markers", name="Normal",
            marker=dict(color="#60a5fa", size=6),
        )
        fig.add_scatter(
            x=[t for t, _ in anomalies], y=[v for _, v in anomalies],
            mode="markers", name="Anomaly",
            marker=dict(color="#f87171", size=9, symbol="circle-open",
                        line=dict(width=2, color="#f87171")),
        )
        fig.add_hline(
            y=threshold, line_dash="dash", line_color="#f87171", line_width=1.5,
