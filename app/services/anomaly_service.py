...
class AnomalyService:
    ...
    async def train(self, series_id: str, data: TrainData) -> TrainResponse:
        ...
        async with self._get_lock(series_id):
            ...
            persistence.save_training_data(
                series_id, version, data.timestamps, data.values
            )
    ...
    async def health(self) -> HealthCheckResponse:
        ...
        return HealthCheckResponse(
            ...
            inference_latency_ms=LatencyMetrics(
                avg=self._infer_latency.avg(), p95=self._infer_latency.p95()
            ),
            training_latency_ms=LatencyMetrics(
                avg=self._train_latency.avg(), p95=self._train_latency.p95()
            ),
        )
